...
//  Calculator
//
//  Created by Luiz Lima on 27/03/23.
//
...
struct ContentView: View {
    ...
    func removeZerosFromTheEnd(value: Double) -> String {
    ...
    func process(digit: Any) {
        if operation > 0 {
            result = 0
            previousOperation = operation
            operation = -1
        }
        if let digit = digit as? Int {
            if decimal > 0.0 {
                ...
                values = removeZerosFromTheEnd(value: result)
            }
        } else if digit as? String == "." {
            decimal = 10.0
            values = "\(values)."
        }
    }
        ...
        VStack(alignment: .trailing, spacing: 0) {
            ...
            HStack(spacing: 0) {
                Text(values)
                    ...
                    .font(.system(
                        size: CGFloat(
                            80 / Int((Double(String(result).count + 10) / 8.0))
                        )))
